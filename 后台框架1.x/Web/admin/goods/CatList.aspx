<%@ Page Title="" Language="C#" MasterPageFile="~/admin/AdminMaster.Master" AutoEventWireup="true" CodeBehind="CatList.aspx.cs" Inherits="hm.Web.admin.goods.CatList" %>
...
<script>
    ...
    function addRole() {
        ...
        ajax("/admin/goods/CatList.aspx?act=add&t=" + Math.random(), pars, function (ret) {
            if (ret.status == 1) {
    ...
    function editShow(id) {
        var pars = { siId: id };
        ajax("/admin/goods/CatList.aspx?act=editshow&t=" + Math.random(), pars, function (ret) {
            if (ret.status == 1) {
                $("#id_edit").val(id);
    ...
    function editRole() {
        ...
        ajax("/admin/goods/CatList.aspx?act=editsubmit&t=" + Math.random(), pars, function (ret) {
            if (ret.status == 1) {
                location.href = location.href;
            }
            else {
    ...
    function del(catId) {
        if (confirm('是否删除？')) {
            ...
            ajax("/admin/goods/CatList.aspx?act=del&t=" + Math.random(), pars, function (ret) {
                if (ret.status == 1) {
                    location.href = location.href;
                }
                else {
